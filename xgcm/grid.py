...
class Grid:
    ...
    def transform(
        ...
        def _check_other_dims(target_da):
            ...
            da_other_dims = set(da.dims) - set(ax.coords.values())
            target_da_other_dims = set(target_da.dims) - set(ax.coords.values())
        ...
        _, dim = ax._get_position_name(da)
        if method == "linear":
            ...
            try:
                target_data_dim = ax.coords["outer"]
            except KeyError:
                raise RuntimeError(
            ...
            if target_data_dim not in target_data.dims:
                ...
                target_data = self.interp(target_data, axis, boundary="extend")
                ...
                target_data = target_data.chunk(
                    {ax._get_position_name(target_data)[1]: -1}
                )
